...
def add_event(event_date: str, title: str, asset_code=None, note=None) -> None:
    get_client().table("events").insert({
        "event_date": event_date, "title": title,
        "asset_code": asset_code, "note": note,
    }).execute()


def get_events(from_date: str, to_date: str) -> list:
    res = (
        get_client().table("events").select("*")
        .gte("event_date", from_date).lte("event_date", to_date)
...
def delete_event(event_id: str) -> None:
    get_client().table("events").delete().eq("id", event_id).execute()


# ---------- Dossier-Historie (v3) ----------

def snapshot_dossiers() -> None:
    """Speichert den heutigen Stand aller Dossiers (1x pro Tag, ueberschreibt denselben Tag)."""
    from datetime import date
    today = date.today().isoformat()
    client = get_client()
    for d in get_dossiers():
        client.table("dossier_history").upsert({
            "snap_date": today, "asset_code": d["code"],
            "direction": d["direction"], "confidence": d["confidence"],
            "thesis": d["thesis"],
            "narrative_score": d.get("narrative_score", 0),
        }, on_conflict="snap_date,asset_code").execute()


def get_history_for_date(snap_date: str) -> list:
    res = (
        get_client().table("dossier_history").select("*")
        .eq("snap_date", snap_date).order("asset_code").execute()
    )
    return res.data or []


def get_history_for_asset(asset_code: str, limit: int = 60) -> list:
    res = (
        get_client().table("dossier_history").select("*")
        .eq("asset_code", asset_code).order("snap_date", desc=True).limit(limit).execute()
    )
    return res.data or []
...
def add_recap(recap_date: str, content: str) -> None:
    get_client().table("recaps").insert({"recap_date": recap_date, "content": content}).execute()


def get_recaps(limit: int = 30) -> list:
    res = (
        get_client().table("recaps").select("*")
        .order("recap_date", desc=True).limit(limit).execute()
    )
    return res.data or []


def set_recap_note(recap_id: str, user_note: str) -> None:
    get_client().table("recaps").update({"user_note": user_note}).eq("id", recap_id).execute()
