...
def _full_context() -> str:
    ...
    return prompts.build_context(
        ...
        upcoming_events=db.get_events(today.isoformat(), (today + timedelta(days=14)).isoformat()),
    )
...
def analyze_input(files: list, pasted_text: str, note: str) -> dict:
    ...
    for f in (files or []):
        if len(f.getvalue()) > 9 * 1024 * 1024:
            raise ValueError(f"Datei '{f.name}' ist groesser als 9 MB - bitte verkleinern oder zuschneiden.")
    ...
    for a in result.get("assets", []):
        ...
        if code not in valid_codes:
            continue
        touched.append(code)
        old = next((d for d in db.get_dossiers() if d["code"] == code), {})
        fields = {
            "direction": a.get("direction", "Neutral"),
            "confidence": _safe_int(a.get("confidence"), old.get("confidence", 0)),
        ...
        if a.get("spot_note"):  # Marktstand nur ueberschreiben, wenn neuer Kurs erkannt wurde
            fields["spot_note"] = a["spot_note"]
        if a.get("priced_in"):  # Einpreisung nur bei neuer Info aktualisieren
            fields["priced_in"] = a["priced_in"]
        if a.get("yield_10y"):  # 10J-Rendite nur bei neuer Info aktualisieren
            fields["yield_10y"] = a["yield_10y"]
        try:
            fields["narrative_score"] = max(-3, min(3, int(a.get("narrative_score", 0))))
        except (TypeError, ValueError):
            fields["narrative_score"] = old.get("narrative_score", 0)
        db.update_dossier(code, fields)
        drv = a.get("driver") or {}
        if drv.get("title"):
    ...
    # 4) Kommende Events aus Kalender-Daten uebernehmen
    for ev in result.get("events", []) or []:
        if ev.get("date") and ev.get("title"):
            db.add_event(ev["date"], ev["title"], ev.get("asset_code"), ev.get("note"))

    # 5) Tages-Schnappschuss aller Dossiers (fuer die Historie)
    try:
        db.snapshot_dossiers()
    except Exception:
        pass  # Historie darf die Analyse nie blockieren
...
def generate_recap() -> str:
    """Tages-Recap: Key Events, Marktton, Reaktion, Narrative, Ausblick morgen."""
    from datetime import date
    db.snapshot_dossiers()
    system = prompts.identity() + "\n\n" + _full_context()
    instructions = prompts.RECAP_INSTRUCTIONS.format(today=date.today().strftime("%d.%m.%Y"))
    resp = get_ai().messages.create(
        model=MODEL, max_tokens=MAX_TOKENS, system=system,
        messages=[{"role": "user", "content": instructions}],
    )
    text = "".join(b.text for b in resp.content if b.type == "text").strip()
    db.add_recap(date.today().isoformat(), text)
    return text
